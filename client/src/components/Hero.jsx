  ...
  return (
    ...
    transition={{duration:0.8}}
     className='h-screen flex flex-col items-center justify-center gap-14 bg-light text-center' >
       ...
       <motion.form 
        initial = {{scale:.95,opacity:0,y:50}}
      ...
      <div className='flex flex-col md:flex-row items-start md:items-center gap-10 min-md:ml-8' >
        <div className='flex flex-col items-start gap-2' >
            <select  required value={pickupLocation} onChange={(e)=>setPickupLocation(e.target.value)} > 
                ...
                <p className='px-1 text-sm text-gray-500'>{pickupLocation?pickupLocation:'Plaease Select Location'}</p>
        </div>
       ...
       whileTap= {{ scale:0.95}}
          className='flex items-center justify-center gap-1 px-9 py-3 max-sm:mt-4 bg-primary hover:bg-primay-dull text-white rounded-full cursor-pointer' >
